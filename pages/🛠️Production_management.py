...
with col_5:
  ...
  if check :
    ...
    st.markdown('''문제를 해결하기 위해 Excel의 Solver 도구를 사용하여 최적화를 수행했습니다. 
    목표는 장난감과 각 하위조립품 쌍의 생산률을 최대화하여 총 이익을 극대화하는 것이었습니다. 
...
with col_7:
  ...
  if check :
    ...
    st.markdown('''문제를 해결하기 위해 Excel의 Solver 도구를 사용하여 최적화를 수행했습니다. 
    목표는 두 가지의 변수를 이용하여 총 비용을 최소화하는 것입니다. 
...
with col_02:
  ...
  ### 📘 생산운영관리
  **유성열 , 최성훈 저자(글)**
  최근 들어서는 생산운영관리가 글로벌화 및 서비스업의 확대 등으로 인해 그 범위가 점점 확대되어 가고 있다. 
...
button = st.button('다 했어요✨') # 버튼은 클릭 여부를 반환
if button:
    st.balloons()
    st.success('제 소개는 여기서 끝이에요! 감사합니다!')
