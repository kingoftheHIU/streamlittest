...
with col_01:
  ...
  st.text('현대 사회에서 개발자와 프로그래머가 갖추어야 하는 필수 소양은 알고리즘을 이용한 문제 해결 능력이다. 이 책은 문제의 성격에 따라 적합한 알고리즘을 소개하고 그 작동 원리를 설명한다. 특히 현실 세계의 문제를 해결하는 40여 개 알고리즘을 보여주면서 그 작동 원리를 이해시키고 파이썬으로 직접 구현해 본다.')

with col_02:
  st.image('pages/book2.jpg')
  st.text('생산운영관리는 회계, 재무, 마케팅, 인사조직, 경영정보와 더불어 기업경영의 핵심적인 기능으로 생산시스템의 설계와 운영에 대해 다루고 있는 학문 영역이다. 지금까지의 생산운영관리가 고전적인 대량생산 체제하에서 제조업 중심의 생산성 제고 및 비용 절감에 초점을 맞춘데 비해, 최근 들어서는 글로벌화 및 서비스업의 확대 등으로 인해 그 범위가 점점 확대되어 가고 있다. 또한 생산운영을 위한 보다 효율적이고 효과적인 새로운 기법들도 지속적으로 출현하고 있다.
